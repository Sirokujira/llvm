...
RX600TargetLowering::
RX600TargetLowering(RX600TargetMachine &TM)
  : TargetLowering(TM, new TargetLoweringObjectFileELF()),
    Subtarget(*TM.getSubtargetImpl()) {
  DEBUG(dbgs() << ">> RX600TargetLowering::constructor <<\n");
  ...
  // ターゲットで利用できるレジスタを登録
  addRegisterClass(MVT::i32, &RX600::CPURegsRegClass);
  ...
  // スタックポインタのレジスタを指定
  setStackPointerRegisterToSaveRestore(RX600::SP);

  // レジスタの操作方法を計算
  computeRegisterProperties();
}

SDValue RX600TargetLowering::
...
#include "RX600GenCallingConv.inc"
...
LowerFormalArguments(SDValue Chain, CallingConv::ID CallConv,
  ...
  DEBUG(dbgs() << ">> RX600TargetLowering::LowerFormalArguments <<\n");
  ...
  CCInfo.AnalyzeFormalArguments(Ins, CC_RX600);
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      const TargetRegisterClass *RC = &RX600::CPURegsRegClass;
...
LowerCall(CallLoweringInfo &CLI,
  ...
  DEBUG(dbgs() << ">> RX600TargetLowering::LowerCall <<\n");
  ...
  CCInfo.AnalyzeCallOperands(Outs, CC_RX600);
  ...
  InChain = DAG.getNode(RX600ISD::Call, dl, NodeTys, &Ops[0], Ops.size());
  InFlag = InChain.getValue(1);
...
LowerCallResult(SDValue Chain, SDValue InFlag,
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_RX600);
...
LowerReturn(SDValue Chain,
  ...
  DEBUG(dbgs() << ">> RX600TargetLowering::LowerReturn <<\n");
  DEBUG(dbgs() << " Chain: "; Chain->dumpr(););
  ...
  // 戻り値を解析する
  CCInfo.AnalyzeReturn(Outs, RetCC_RX600);
  ...
  // 常に "ret $ra" を生成
  if (Flag.getNode())
    return DAG.getNode(RX600ISD::Ret, dl, MVT::Other,
                       Chain, DAG.getRegister(RX600::RA, MVT::i32), Flag);
  else // Return Void
    return DAG.getNode(RX600ISD::Ret, dl, MVT::Other,
                       Chain, DAG.getRegister(RX600::RA, MVT::i32));
}
