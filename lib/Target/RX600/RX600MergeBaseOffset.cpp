//===----- RX600MergeBaseOffset.cpp - Optimise address calculations  ------===//
//
...
struct RX600MergeBaseOffsetOpt : public MachineFunctionPass {
  static char ID;
  ...
  RX600MergeBaseOffsetOpt() : MachineFunctionPass(ID) {}
  ...
  StringRef getPassName() const override {
    return RX600_MERGE_BASE_OFFSET_NAME;
  }
...
bool RX600MergeBaseOffsetOpt::detectLuiAddiGlobal(MachineInstr &HiLUI,
                                                  MachineInstr *&LoADDI) {
  if (HiLUI.getOpcode() != RX600::LUI ||
      HiLUI.getOperand(1).getTargetFlags() != RX600II::MO_HI ||
  ...
  if (LoADDI->getOpcode() != RX600::ADDI ||
      LoADDI->getOperand(2).getTargetFlags() != RX600II::MO_LO ||
...
// output from LoADDI.
void RX600MergeBaseOffsetOpt::foldOffset(MachineInstr &HiLUI,
                                         MachineInstr &LoADDI,
...
bool RX600MergeBaseOffsetOpt::matchLargeOffset(MachineInstr &TailAdd,
                                               unsigned GAReg,
                                               int64_t &Offset) {
  assert((TailAdd.getOpcode() == RX600::ADD) && "Expected ADD instruction!");
  ...
  if (OffsetTail.getOpcode() == RX600::ADDI) {
    ...
    if (AddiImmOp.getTargetFlags() != RX600II::MO_None)
      return false;
    ...
    if (OffsetLui.getOpcode() != RX600::LUI ||
        LuiImmOp.getTargetFlags() != RX600II::MO_None ||
    ...
    return true;
  } else if (OffsetTail.getOpcode() == RX600::LUI) {
...
bool RX600MergeBaseOffsetOpt::detectAndFoldOffset(MachineInstr &HiLUI,
                                                  MachineInstr &LoADDI) {
...
bool RX600MergeBaseOffsetOpt::runOnMachineFunction(MachineFunction &Fn) {
...
FunctionPass *llvm::createRX600MergeBaseOffsetOptPass() {
  return new RX600MergeBaseOffsetOpt();
}
